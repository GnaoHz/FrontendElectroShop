        ...
        HStack {
            ...
            VStack(alignment: message.isCustomer ? .trailing : .leading, spacing: 5) {
                Text(message.content)
                    .padding(10)
                    .background(message.isCustomer ? Color.blue : Color.gray.opacity(0.2))
...
struct CustomerSupportChatView: View {
    ...
    var body: some View {
        VStack {
            ...
            HStack {
                TextField("Nhập tin nhắn...", text: $inputMessage, axis: .vertical)
        ...
        .navigationTitle("Hỗ trợ Trực tuyến")
    }
    ...
    private func simpleAIBotResponse(for message: String) -> String {
        ...
        if lowercased.contains("cảm ơn") || lowercased.contains("cám ơn") {
            ...
            return "Bạn vui lòng cung cấp mã sản phẩm hoặc tên sản phẩm để chúng tôi kiểm tra thông tin giá và chi tiết nhé."
        } else if lowercased.contains("giao hàng") || lowercased.contains("đơn hàng") {
            return "Bạn vui lòng cung cấp mã đơn hàng để chúng tôi kiểm tra tình trạng giao hàng cho bạn."
        } else {
            return "Chúng tôi đã nhận được tin nhắn của bạn. Vui lòng chờ trong giây lát, nhân viên hỗ trợ sẽ phản hồi bạn ngay."
        }
    }
}
